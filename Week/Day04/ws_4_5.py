...
def is_validation(blood, mail, name, website):    
    what_error = []

    # website 존재
    if not website or len(website) < 1:
        what_error.append('website')

    # name 길이
    if not (2 <= len(name) <= 30):
        what_error.append('name')

    # mail에 @ 포함 여부
    if '@' not in mail:
        what_error.append('mail')
    
    # blood_group 유효성
    if blood not in blood_types:
        what_error.append('blood_group')

    if what_error:
        return False, what_error
    else:
        return True, []


def create_user(user_list_data):
    global error_count
    for user in user_list_data:
        ...
        valid, errors = is_validation(blood, mail, name, website)
        if not valid:
            error_count += 1
            for field in errors:
                user_entry[field] = None

        user_list.append(user_entry)

    print(f'잘못된 데이터로 구성된 유저의 수는 {error_count}명입니다.')
    pprint(user_list)
